    ...
    void write_to_file(int aesd_fd, const char *data, size_t length) {
        ssize_t bytes_written = write(aesd_fd, data, length); // Write data to file
        if (bytes_written < 0) {
            LOG_ERR("Failed to write to file: %s", strerror(errno));
            return; // Return if writing fails
        }
        if (bytes_written < length) {
            LOG_ERR("Not all data written to file, expected %zu bytes but wrote %zd bytes", length, bytes_written);
        }
    }
    ...
    size_t read_from_file(int aesd_fd, char *buffer, size_t buffer_size) {
        size_t bytes_read = read(aesd_fd, buffer, buffer_size - 1); // Read data from file
        if (bytes_read < 0) {
            LOG_ERR("Failed to read from file: %s", strerror(errno));
            return 0; // Return if reading fails
        }
    ...
    void stream_file_reader(int aesd_fd, int client_socket) {
        ...
        while ((bytes_read = read(aesd_fd, buffer, sizeof(buffer) - 1)) > 0) {
            ...
            while(total_sent < bytes_read) {
                ssize_t sent = send(client_socket, buffer + total_sent, bytes_read - total_sent, MSG_NOSIGNAL);
                if (sent < 0) {
        ...
        if (bytes_read < 0) {
            ...
            LOG_SYS("End of file reached, no more data to send");
        }
    }
